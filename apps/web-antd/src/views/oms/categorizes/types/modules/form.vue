...
import { createCategorizeType, updateCategorizeType } from '#/api';
...
const getTitle = computed(() => {
  return formData.value?.id
    ? $t('ui.actionTitle.edit', ['类型'])
    : $t('ui.actionTitle.create', ['类型']);
});
    ...
    if (valid) {
      ...
      try {
        await (formData.value?.id
          ? updateCategorizeType(formData.value.id, data)
          : createCategorizeType(data));
        modalApi.close();
        emit('success');
      } finally {
        modalApi.lock(false);
